...
Keep responses thoughtful, warm, concise, and focused on self-discovery. Ask insightful open-ended follow-up questions.`
    ...
    return res.json({ 
      reply: replyText,
      notice: 'Served via contextual fallback.'
    });
  }
});
...
`;
    ...
    console.error('[Summarize Fallback Active]:', error.message);
    ...
    return res.json({
      ...
      actionItems: [
        ...
        'Ensure a standard Google AI Studio key (starting with AIzaSy) is configured in Cloud Run.'
      ]
    });
  }
});
...
`;
    ...
    console.error('[Cognitive Insights Fallback Active]:', error.message);
    return res.json({
